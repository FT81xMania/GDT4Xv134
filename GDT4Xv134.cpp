...
void Bitmap::fromtext(int font, const char* s)
{
  ...
  GD.wr32(REG_HSIZE, hsize);
  GD.wr32(REG_VSIZE, vsize);
  GD.wr32(REG_PCLK, pclk);
  defaults(ARGB4);
}
...
uint32_t GDClass::measure_freq(void)
{
  ...
  //Serial.println(GDTR.rd32(REG_CLOCK));
...
LOW_FREQ_BOUND = 46040000UL;  //
}

if (SizeFT813==53){                    //BT815
LOW_FREQ_BOUND = 47040000UL;  //
}
...
LOW_FREQ_BOUND = 60000000UL;  //  59000000UL   óptimo
 ...
 //Serial.print("model: ");   Serial.println(ft8xx_model);
 Serial.println("****");
 ...
 //Serial.println(rd32(REG_HOFFSET));
  ...
  //GDTR.wr32(REG_FREQUENCY, 72000000UL);
  Serial.print("REG_FREQUENCY: ");   Serial.println(REG_FREQUENCY);//Serial.println(f);
  ...
  //Serial.println(rd32(REG_FREQUENCY));
  //Serial.println(rd32(REG_CLOCK));
  Serial.println("****");
  Serial.println(" ");
}
...
if (SizeFT813==51){
 ...
 //   GD.wr32(REG_HSIZE, 800);
 //   GD.wr32(REG_HCYCLE, 1125);   //990 1000 1056  One Horizontal Line    Th    afecta parpadeo en lineas en gráficas el típico parpadea excesivamente
 //	  GD.wr32(REG_HSYNC1, 48);    //10    HS pulse width         Thpw   usar el típico del datasheet
 //   GD.wr32(REG_HOFFSET, 23+46);   //46    HS Blanking         Thb  controla desplazamiento a la derecha       -->lo restas acá
 ...
 //   GD.wr32(REG_VOFFSET, 13+23);   //23  VS Blanking      Tvb    controla desplazamiento vertical hacia abajo    -->lo restas acá 
...
//TFT Riverdi 5"             EVE4     Datasheet Rev.0.1 2020-12-29   p.15/21
if (SizeFT813==54){
	cmd_setrotate(ORIENTACION);
...
void GDClass::cmd_fillwidth(uint32_t s) {
  cFFFFFF(0x58);
  cI(s);
}
